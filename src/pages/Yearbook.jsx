...
function Yearbook() {
  ...
  const filteredFriends =
    selectedYear === "All"
      ? quotes.filter((friend) =>
          friend.user.name.toLowerCase().includes(searchText.toLowerCase())
        )
      : quotes.filter(
          (friend) =>
            friend.user.rollNumber === selectedYear &&
            friend.user.name.toLowerCase().includes(searchText.toLowerCase())
        );
    ...
    switch (type) {
      ...
      case "yearAsc":
        sortedFriends.sort((a, b) =>
          a.user.rollNumber.localeCompare(b.user.rollNumber)
        );
        break;
      case "yearDesc":
        sortedFriends.sort((a, b) =>
          b.user.rollNumber.localeCompare(a.user.rollNumber)
        );
        break;
      default:
        break;
    ...
    const getQuotes = async () => {
      try {
        const res = await axios.get(
          "https://odyssey-iota-vert.vercel.app/api/quote"
        );
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br pt-20 from-gray-900 via-blue-900 to-purple-900 animate-text text-white md:px-10 lg:px-20">
        {/* Animated background */}
        <svg
            ...
            <div className="flex items-center space-x-2 justify-center">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="outline"
                    className="p-2 bg-sky-800 text-white border-2 border-sky-600 hover:bg-sky-700 hover:border-sky-500"
                  >
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 md:gap-2">
              {quotes &&
                filteredFriends.map((quote) => (
                  ...
                  >
                    <Link
                      to={`/yearbook/${quote.user.rollNumber}`}
                      key={quote._id}
